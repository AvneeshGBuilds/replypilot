    ...
    <main className="min-h-screen bg-[#f5f5f8]">
      ...
      <nav className="bg-white/80 backdrop-blur border-b border-gray-100 px-6 py-3.5 flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-2.5">
          ...
          {businessName && (
            <><span className="text-gray-200">/</span><span className="text-sm text-gray-400 font-medium">{businessName}</span></>
          )}
        </div>
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => auth.signOut()} className="text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-100 px-3 py-1.5 rounded-lg transition font-medium">Sign out</button>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <AnimatePresence>
          {!businessName && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-amber-100 rounded-xl flex items-center justify-center flex-shrink-0">
                  <svg className="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
                </div>
                <div>
                  <p className="text-sm font-semibold text-amber-900">Set up your business first</p>
                  <p className="text-xs text-amber-700 mt-0.5">Add your business name so replies are personalized</p>
                </div>
              </div>
              <a href="/settings" className="flex-shrink-0 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition">Setup →</a>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {reviews.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-3 gap-3 mb-5">
              <StatCard label="Reviews" value={String(reviews.length)} icon="📋" />
              <StatCard label="Response rate" value={`${responseRate}%`} icon="✅" highlight={responseRate === 100} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">{autoPost ? "Replies go live on Google automatically" : "You approve each reply before it posts"}</p>
            </div>
          </div>
          <button onClick={toggleAutoPost} disabled={savingToggle}
            ...
            <motion.span layout className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm`}
              animate={{ x: autoPost ? 24 : 4 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
          </button>
        </div>
          ...
          {addingTest && (
            <motion.div initial={{ opacity: 0, scale: 0.97, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97 }}
              ...
              <div className="flex items-center justify-center gap-2 mt-4">
                ...
                <p className="text-xs text-gray-400 font-medium">Generating AI reply…</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {pending.length === 0 && posted.length === 0 && !addingTest && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              ...
              <p className="text-sm text-gray-400 mb-6 max-w-xs mx-auto">New Google reviews will appear here with AI replies ready to approve</p>
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={addTestReview}
          ...
          <section className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <motion.span layout className="bg-violet-100 text-violet-700 text-xs font-bold px-2 py-0.5 rounded-full">{pending.length}</motion.span>
              </div>
              {!addingTest && (
                <button onClick={addTestReview} className="text-xs text-gray-400 hover:text-violet-600 transition font-medium">+ Add test</button>
              )}
            </div>
            <AnimatePresence mode="popLayout">
              {pending.map((r) => (
                <ReviewCard key={r.id} review={r} editingId={editingId} editText={editText} posting={posting} deleting={deleting}
        ...
        {posted.length > 0 && (
          <section>
            ...
            <AnimatePresence mode="popLayout">
              {posted.map((r) => (
                <ReviewCard key={r.id} review={r} editingId={null} editText="" posting={null} deleting={deleting}
                  onEdit={() => {}} onEditChange={() => {}} onSaveEdit={() => {}} onCancelEdit={() => {}} onApprove={() => {}}
                  onDelete={() => deleteReview(r.id)} />
              ))}
            </AnimatePresence>
          </section>
...
function StatCard({ label, value, icon, highlight }: { label: string; value: string; icon: string; highlight?: boolean }) {
  return (
    <motion.div whileHover={{ y: -2 }} className={`bg-white rounded-2xl border shadow-sm p-4 cursor-default ${highlight ? "border-emerald-200 bg-gradient-to-br from-emerald-50 to-white" : "border-gray-100"}`}>
...
function ReviewCard({ review, editingId, editText, posting, deleting, onEdit, onEditChange, onSaveEdit, onCancelEdit, onApprove, onDelete }: {
  review: Review; editingId: string | null; editText: string; posting: string | null; deleting: string | null;
  onEdit: () => void; onEditChange: (v: string) => void; onSaveEdit: () => void; onCancelEdit: () => void; onApprove: () => void; onDelete: () => void;
}) {
    ...
    >
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${ratingBadge(review.rating)}`}>
              {"★".repeat(review.rating)}{"☆".repeat(5 - review.rating)}
            </span>
            {review.status === "posted" && (
              <span className="text-xs bg-emerald-50 text-emerald-600 border border-emerald-100 px-2.5 py-1 rounded-full font-bold">Posted ✓</span>
            )}
          </div>
        </div>

        {/* Review text */}
        <p className="text-gray-500 text-sm leading-relaxed mb-4">"{review.reviewText}"</p>
          ...
          <AnimatePresence mode="wait">
            {isEditing ? (
              ...
              <motion.p key="text" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="text-sm text-gray-700 leading-relaxed">{review.suggestedReply}</motion.p>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="flex gap-2 items-center">
          {review.status !== "posted" && (
            <>
              {isEditing ? (
                <>
                  <motion.button whileTap={{ scale: 0.95 }} onClick={onSaveEdit} className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-3.5 py-2 rounded-lg font-semibold transition">Save</motion.button>
                  <button onClick={onCancelEdit} className="text-xs text-gray-400 hover:text-gray-600 px-2 py-2 transition">Cancel</button>
                </>
              ) : (
                <motion.button whileTap={{ scale: 0.95 }} onClick={onEdit} className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-3.5 py-2 rounded-lg font-semibold transition flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1.5">
            <AnimatePresence mode="wait">
              {confirmDelete ? (
                <motion.div key="confirm" initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => setConfirmDelete(false)} className="text-xs text-gray-400 hover:text-gray-600 px-2 py-1.5 transition">No</button>
                </motion.div>
              ) : (
